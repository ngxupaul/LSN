...
<style>
  ...
  #tools { position:absolute; z-index:1000; top:10px; left:52px; display:flex; gap:6px; flex-wrap:wrap; max-width:620px; }
  ...
  #filters { position:absolute; z-index:1000; top:64px; left:52px; display:flex; gap:6px; }
  #filters button { background:#fff; border:1px solid #ccc; border-radius:4px; padding:5px 10px;
                    font:12.5px sans-serif; cursor:pointer; box-shadow:0 1px 5px rgba(0,0,0,.35); }
  #filters button.active { background:#ff9d9d; border-color:#e74c3c; }
  #filters button.active.blue { background:#aed6f1; border-color:#2980b9; }
  #filters button.active.purple { background:#d7b8ef; border-color:#8e44ad; }
  #searchBox { position:absolute; z-index:1000; top:100px; left:52px; width:280px;
               font:13px sans-serif; padding:6px 8px; border:1px solid #ccc; border-radius:4px;
               box-shadow:0 1px 5px rgba(0,0,0,.4); }
  #listPanel, #statsPanel { position:absolute; z-index:1000; top:135px; left:52px; width:350px; max-height:60%;
  ...
  #statsPanel { left:415px; }
  #listPanel h3, #statsPanel h3 { margin:0; padding:8px 10px; background:#f5f5f5; border-bottom:1px solid #ddd; font-size:14px; }
  ...
  #listPanel .row.to { background:#fffdf0; }
  #listPanel .row.to b { color:#b7791f; }
  #statsPanel table { width:100%; border-collapse:collapse; font-size:12.5px; }
  #statsPanel th, #statsPanel td { padding:4px 6px; border-bottom:1px solid #eee; text-align:right; }
  #statsPanel th { background:#fafafa; }
  #statsPanel td:first-child, #statsPanel th:first-child { text-align:left; }
  #statsPanel tr.total td { font-weight:bold; background:#fff7d6; }
</style>
</head>
<body>
<div id="map"></div>
<div id="tools">
  ...
  <button id="btnClear" title="Xóa toàn bộ dữ liệu">🗑 Xóa hết</button>
...
function houseInfo(l) {
  ...
  return '<div style="min-width:240px">' +
    '<b>🏠 ' + (esc(p['addr:housenumber']) ? 'Số ' + esc(p['addr:housenumber']) + ' — ' : '') +
      (esc(p.name) || 'Chưa có tên') + flag + '</b><br>' +
    ...
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Nhân khẩu</td><td>' + (esc(p.members) || '—') + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Người cao tuổi</td><td>' + (esc(p.elderly) || '—') + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Trẻ em</td><td>' + (esc(p.children) || '—') + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Hộ chính sách</td><td>' + (esc(p.policy) || '—') + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Cần hỗ trợ</td><td>' + (esc(p.support) || '—') + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Loại</td><td>' + typeName(p.building) + '</td></tr>' +
...
function toInfo(l) {
  const p = props(l);
  const s = totStats(l);
  return '<div style="min-width:230px"><b>🏘 ' + esc(p.name || 'Tổ') + '</b><br>' +
    '<table style="border-collapse:collapse;font-size:13px">' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Số hộ</td><td><b>' + s.to + '</b></td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Nhân khẩu</td><td><b>' + s.mem + '</b></td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Người cao tuổi</td><td>' + s.eld + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Trẻ em</td><td>' + s.kid + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Hộ chính sách</td><td>' + s.pol + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Hộ cần hỗ trợ</td><td>' + s.sup + '</td></tr>' +
    '<tr><td style="padding:2px 8px 2px 0;color:#666">Công trình</td><td>' + s.ct + '</td></tr>' +
    '</table>' +
    '<button onclick="renameTo(this)" style="margin-top:6px">✏️ Đổi tên</button> ' +
